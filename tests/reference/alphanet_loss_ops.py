"""Vendored from github.com/facebookresearch/AlphaNet at 1df57bf

Unmodified. Licensed CC BY-NC 4.0, the same terms as the US-Net snapshot
this repository is built on. Present so that the equivalence check in
tests/test_loss_ops.py runs anywhere the repository does, rather than only
where someone happened to clone the reference.

Not imported by training code. Only the test reads it.
"""
