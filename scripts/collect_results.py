...
def render():
    ...
    w('')
    w('And they do it in one epoch, without being asked. BF and BG '
      'permute nothing, and the epoch after the narrow widths arrive '
      'they read 0.744 and 0.731. The sandwich rule sorts its own '
      'prefix immediately: weight[:k] takes gradient at every width '
      'and has to classify alone at 0.25, and one epoch of that is '
      'enough. A permutation at that moment buys 0.10 of overlap, '
      'which is one epoch of head start, and by the end the permuted '
    ...
    w('Which is why sorting is worth nothing here, and the accuracy '
      ...
      'and the ordering stays arbitrary until something sorts it. '
      'US-Net asks from step one, so there is nothing left to sort.')
    w('')
    ...
    w('- What AW is still worth, and a correction to what this page '
      ...
      'configs and they differ by one line, `teacher_chain: True`, '
      'and that flag only changes which soft target the logits are '
      'matched against; the transport term runs inside AW untouched, '
      'with train.py restoring the width order so the pairing is '
      'the same one K uses. The ladder is additive: A, plus transport, '
      'is K at +0.75 and ahead at all sixteen widths; K, plus '
      'chaining, is AW at +0.06. So chaining on top of transport buys '
      'nothing in accuracy. What it does move is NLL, 1.127 to 1.091, '
      'the largest such gain of anything stacked on K, and the '
      'mechanism is plain enough: a target from the adjacent width is '
      'closer than one from the widest, so the student is less '
      'over-confident. Better calibration, not better accuracy.')
    w('- Whether chaining does anything on its own, which has never '
      'been measured. `teacher_chain` appears in exactly two configs '
      'and both are AW, so every reading of it sits on top of the '
      'transport term. That leaves one cell of a two by two empty: '
      'widest-teacher and chained-teacher, crossed with transport and '
      'no transport, has A at 73.52, K at 74.26, AW at 74.32 and '
      'nothing at all for chaining alone. BJ is that cell - A with '
      'teacher_chain and no transport anywhere. Near 73.5 and the '
      'transport term is the only active ingredient. Near 74.3 and '
      'the two are redundant rather than additive, which changes what '
      'this report claims.')
    w('- That K is saturated, which is now four branches deep. Every '
      'addition to K has cost accuracy except the one that did '
      'nothing: AE at -0.84, AH at -0.73, AF at -0.23, AW at +0.06. '
      'The next idea worth running is unlikely to be another term '
      'added to this loss.')
