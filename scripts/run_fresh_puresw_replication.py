"""GPU worker scheduler for the fresh seed-7 Pure-SW replication."""
...
from rq2_fresh_puresw_replication import CHECKPOINT_EPOCHS, extract_state


def run_workers(root, dataset_root, gate_a_summary, gpu_ids=(0,)):
    root, dataset_root, gate_a_summary = map(Path, (root, dataset_root, gate_a_summary))
    worker_root = root / "state_workers"
    worker_root.mkdir(parents=True, exist_ok=True)
    available, pending, running, timing = list(map(int, gpu_ids)), list(CHECKPOINT_EPOCHS), {}, []
    ...
    def launch(epoch, gpu):
        ...
        if metadata.is_file() and json.loads(metadata.read_text()).get("status") == "FRESH_PAIRWISE_STATE_COMPLETE":
            return 0, 0.0
        command = [
            sys.executable, "-m", "scripts.run_fresh_puresw_replication", "--worker",
    ...
    with ThreadPoolExecutor(max_workers=len(available)) as pool:
        while pending or running:
            while pending and available:
                ...
                print(f"[fresh Pure-SW] epoch {epoch} starting on physical GPU {gpu}", flush=True)
                running[pool.submit(launch, epoch, gpu)] = (epoch, gpu)
            done, _ = wait(running, return_when=FIRST_COMPLETED)
            for future in done:
                epoch, gpu = running.pop(future)
                code, seconds = future.result()
                available.append(gpu)
                timing.append({"epoch": epoch, "physical_gpu": gpu, "minutes": seconds / 60})
                if code:
                    raise RuntimeError(f"Fresh Pure-SW epoch {epoch} worker failed with {code}")
                print(f"[fresh Pure-SW] epoch {epoch} finished in {seconds/60:.1f} min", flush=True)
    pd.DataFrame(timing).sort_values("epoch").to_csv(
        root / "state_extraction_runtime.csv", index=False
    )
...
def main():
    ...
    extract_state(
        args.root, args.epoch, args.dataset_root, args.gate_a_summary,
        args.output, "cuda:0",
    )
